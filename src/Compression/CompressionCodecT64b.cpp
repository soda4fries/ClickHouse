...
public:
    ...
    CompressionCodecT64B(std::optional<TypeIndex> type_idx_, Variant variant_);
...
{
    if (!type_id)
        throw Exception(ErrorCodes::CANNOT_COMPRESS, "T64B codec doesn't support compression without information about column type");
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Type is not supported by T64B codec: {}", static_cast<UInt32>(*type_id));
}
...
TypeIndex deserializeTypeId(uint8_t serialized_type_id)
{
    ...
    throw Exception(ErrorCodes::INCORRECT_DATA, "Bad magic number in T64B codec: {}", static_cast<UInt32>(serialized_type_id));
}


UInt8 codecId()
{
    return static_cast<UInt8>(CompressionMethodByte::T64B);
}
...
/// ============================================================================
/// Transpose helpers — identical to T64
/// ============================================================================
...
/// ============================================================================
/// T64B-specific: delta helpers — multitarget
/// ============================================================================
...
using Variant = CompressionCodecT64B::Variant;
...
UInt32 compressData(const char * src, UInt32 bytes_size, char * dst)
{
    ...
    UInt32 num_bits = getDeltaBitsNumber(static_cast<UInt64>(delta_range));
    if (!num_bits)
...
UInt32 decompressData(const char * src, UInt32 bytes_size, char * dst, UInt32 uncompressed_size)
{
    ...
    if (uncompressed_size % sizeof(T) != 0)
        throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress T64B-encoded data, unexpected uncompressed size ({})"
    ...
    if (bytes_size < header_size)
        throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress T64B-encoded data, data size ({}) is less than the size of T64B header",
                        bytes_size);
    ...
    if (!bytes_size || bytes_size % src_shift)
        throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress T64B-encoded data, data size ({}) is not a multiplier of {}",
    ...
    UInt64 expected = static_cast<UInt64>(num_full) * matrix_size + tail;
...
UInt32 CompressionCodecT64B::doCompressData(const char * src, UInt32 src_size, char * dst) const
{
    ...
    throw Exception(ErrorCodes::CANNOT_COMPRESS, "Cannot compress with T64B codec");
}

UInt32 CompressionCodecT64B::doDecompressData(const char * src, UInt32 src_size, char * dst, UInt32 uncompressed_size) const
{
    if (!src_size)
        throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress T64B-encoded data");
...
uint8_t CompressionCodecT64B::getMethodByte() const
{
    return codecId();
}

CompressionCodecT64B::CompressionCodecT64B(std::optional<TypeIndex> type_idx_, Variant variant_)
    : type_idx(type_idx_)
    , variant(variant_)
{
    if (variant == Variant::Byte)
        setCodecDescription("T64B");
    else
        setCodecDescription("T64B", {make_intrusive<ASTLiteral>("bit")});
}

void CompressionCodecT64B::updateHash(SipHash & hash) const
{
    ...
    auto reg_func = [&](const ASTPtr & arguments, const IDataType * type) -> CompressionCodecPtr
    {
        ...
        if (arguments && !arguments->children.empty())
        {
            if (arguments->children.size() > 1)
                throw Exception(ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE, "T64B support zero or one parameter, given {}",
                                arguments->children.size());

            const auto children = arguments->children;
            const auto * literal = children[0]->as<ASTLiteral>();
            if (!literal)
                throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "Wrong modification for T64B. Expected: 'bit', 'byte')");
            ...
            if (name == "byte")
                ...
                throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "Wrong modification for T64B: {}", name);
        }
        ...
        if (type)
        {
            ...
            if (type_idx == TypeIndex::Nothing)
                throw Exception(
                    ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE, "T64B codec is not supported for specified type {}", type->getName());
        }
        return std::make_shared<CompressionCodecT64B>(type_idx, variant);
    };

    factory.registerCompressionCodecWithType("T64B", codecId(), reg_func);
}
